...
void BehaviorTreeExecutor::registerParameters()
{
  ...
  param_client_ = std::make_shared<rclcpp::AsyncParametersClient>(
    this,
    this->get_name(), rmw_qos_profile_parameters, cbg_one_);
  param_sub_ =
    param_client_->on_parameter_event(
    std::bind(
      &BehaviorTreeExecutor::onParamEvent, this,
      std::placeholders::_1));
...
void BehaviorTreeExecutor::onParamEvent(rcl_interfaces::msg::ParameterEvent::UniquePtr event)
{
  ...
  if (this->get_namespace() == std::string("/")) {
    namespaced_node_name = std::string(this->get_namespace()) + this->get_name();
  } else {
    namespaced_node_name = this->get_namespace() + std::string("/") + this->get_name();
  }
  ...
  if (event->node == namespaced_node_name) {
    for (auto & changed_parameter : event->changed_parameters) {
      RCLCPP_INFO_STREAM(
        this->get_logger(),
        "Changed Parameter : " << changed_parameter.name << "of Value Type: " << std::to_string(
          changed_parameter.value.type) << ", to:  " << changed_parameter.value.string_value);
